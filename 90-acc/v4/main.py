...
def measure_acc():
    ...
    # MEASURE
    #
    global T_meas, T_exec, T_now, T_start, acc, rmsdACC
...
def store_to_SD():
    ...
    with open(filename,'wb') as file:
        for i in range(len(acc)):
            file.write(ustruct.pack('<fff',acc[i][0],acc[i][1],acc[i][2]))
...
def send_file():
    ...
    request = 'set,state{},3;set,rms{},{},{:.6f};'.format(iACC,iACC,timestamp,rmsdACC)
    logger.info('sending request to the sever: "{}"'.format(request))
    response = socket_client.request(request)
