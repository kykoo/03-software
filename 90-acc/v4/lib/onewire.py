...
class DS18X20(object):
    def __init__(self, onewire):
        ...
        for itry in range(10):
            response = self.ow.scan()
            if response:
                break
            else:
                time.sleep(0.5)
        self.roms = [rom for rom in response if rom[0] == 0x10 or rom[0] == 0x28]
        # print('roms---')
        # print(self.roms)
        self.fp = True
        try:
    ...
    def measure(self, rom=None):
        ...
        while iter < 5:
            ...
            ow.write_byte(0xbe)  # Read scratch
            data = ow.read_bytes(9)
            if data[-1] == ow.crc8(data[0:-1]):
                temp = self.convert_temp(rom[0], data)
