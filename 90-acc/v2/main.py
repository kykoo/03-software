...
def store_to_SD():
    ...
    filename = '/sd/data/gcam{}-acc{}-'.format(iCAM,iACC) + datetime_string(T_start)
...
def send_file():
    logger.info("FTP to CTLR: Remote={} ...".format('/sd/data/{}'.format(date_string(T_start))))
    os.chdir('/sd/data')
    ...
    logger.info('available space = {:,}B'.format(os.getfree('/sd')))
